...
RSpec.describe "Api::ContactMethods", type: :request do
  ...
  describe "POST /api/people/:person_id/contact_methods" do
    let(:valid_params) { { contact_method: { method_type: "phone", value: "415-555-0100" } } }

    it "creates a contact method" do
      post "/api/people/#{person.id}/contact_methods", params: valid_params
    ...
    it "returns 422 for missing value" do
      post "/api/people/#{person.id}/contact_methods",
  ...
  describe "DELETE /api/people/:person_id/contact_methods/:id" do
    let!(:contact_method) { person.contact_methods.first }

    it "destroys the contact method" do
      delete "/api/people/#{person.id}/contact_methods/#{contact_method.id}"
